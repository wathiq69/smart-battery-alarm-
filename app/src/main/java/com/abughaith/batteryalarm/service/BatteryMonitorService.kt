...
class BatteryMonitorService : Service() {
    ...
    companion object {
        ...
        const val ACTION_START = "com.abughaith.batteryalarm.START_MONITORING"
        const val ACTION_STOP = "com.abughaith.batteryalarm.STOP_MONITORING"
    ...
    private fun acquireWakeLock() {
        try {
            wakeLock = powerManager.newWakeLock(
                ...
                "SmartBatteryAlarm::MonitorWakeLock"
            )
    ...
    private fun handleBatteryChange(intent: Intent) {
        ...
        Log.d(TAG, "Battery: $percent% charging=$isCharging status=$status")
        ...
        if (isCharging) {
            ...
            if (percent in listOf(20, 30, 40, 50, 60, 70, 80, 90)) {
                if (percent != lastChargingStepAnnounced) {
                    onChargingProgress(percent)
                    lastChargingStepAnnounced = percent
                }
            }

            if (percent == 100 && !lastFullAnnounced) {
                onChargingComplete()
                lastFullAnnounced = true
            }
        } else {
            ...
            if (percent in LOW_THRESHOLDS) {
                if (percent == 15 && !prefs.lowAlertSpoken15) {
                    onLowBattery(percent); prefs.lowAlertSpoken15 = true
                } else if (percent == 10 && !prefs.lowAlertSpoken10) {
                    onLowBattery(percent); prefs.lowAlertSpoken10 = true
                } else if (percent == 5 && !prefs.lowAlertSpoken5) {
                    onLowBattery(percent); prefs.lowAlertSpoken5 = true
                }
            }

            if (percent > 15) {
                if (prefs.lowAlertSpoken15) prefs.lowAlertSpoken15 = false
                if (prefs.lowAlertSpoken10) prefs.lowAlertSpoken10 = false
                if (prefs.lowAlertSpoken5) prefs.lowAlertSpoken5 = false
            }
        }

        lastPercent = percent
        lastCharging = isCharging
        prefs.lastSpokenPercent = percent
        prefs.lastSpokenCharging = isCharging
    }
    ...
    private fun onChargerConnected(percent: Int) {
        ...
        val remainingFraction = (100 - percent) / 100f
        val remainingMin = (remainingFraction * speedHours * 60f).toInt()
    ...
    private fun showAlertNotification(text: String, title: String, importance: Int) {
        val channelId = if (importance >= NotificationManager.IMPORTANCE_HIGH)
            App.CHANNEL_ID_ALERTS else App.CHANNEL_ID_MONITORING
