...
class MainActivity : AppCompatActivity() {
    ...
    private val bgPicker = registerForActivityResult(
        ...
        if (uri != null) {
            try {
                ...
                Toast.makeText(this, "تعذر حفظ الصورة: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun setupClickListeners() {
        ...
        binding.btnMuteTemp.setOnClickListener {
            showMuteDialog()
        }
        binding.btnRefreshApps.setOnClickListener {
            if (appUsage.hasUsageStatsPermission()) {
                ...
                appUsage.openUsageAccessSettings()
            }
        }
        ...
        if (prefs.weatherEnabled && isOnline()) {
            lifecycleScope.launch {
                try {
                    ...
                    val fullMsg = if (weatherInfo != null) {
                        "$baseMsg. حالة الطقس لهذا اليوم هي $weatherInfo"
                    } else {
                        baseMsg
                    }
    ...
    private fun refreshAppsList() {
        ...
        lifecycleScope.launch {
            ...
            if (apps.isEmpty()) {
                Toast.makeText(this@MainActivity, "لا توجد بيانات استخدام بعد", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun showMuteDialog() {
        val items = arrayOf(
            getString(R.string.mute_for_1h),
            getString(R.string.mute_for_3h),
            getString(R.string.mute_until_morning),
            getString(R.string.unmute)
        )
        AlertDialog.Builder(this)
            .setTitle("خيارات الكتم")
    ...
    private fun showSettingsDialog() {
        val items = arrayOf(
            ...
            "تفعيل/تعطيل أوقات الهدوء (الحالي: ${if (prefs.quietHoursEnabled) "مفعل" else "معطل"})",
            "تفعيل/تعطيل الطقس (الحالي: ${if (prefs.weatherEnabled) "مفعل" else "معطل"})",
    ...
    private fun showOwnerNameDialog() {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("حفظ") { _, _ ->
                prefs.ownerName = input.text.toString().ifBlank { "أبو غيث" }
    ...
    private fun showChargeSpeedDialog() {
        ...
        AlertDialog.Builder(this)
            .setTitle("سرعة الشاحن")
            .setItems(items) { _, which ->
                prefs.chargeSpeedHours = values[which]
                Toast.makeText(this, "تم الحفظ", Toast.LENGTH_SHORT).show()
            }
            .show()
    }
